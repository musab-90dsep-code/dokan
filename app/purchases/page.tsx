...
export default function PurchasesPage() {
  ...
  // Dynamic Category Product Creation States (Matching User Images 1, 2, 3)
  const [entryMode, setEntryMode] = useState<'category' | 'select'>('category');
  const [itemCategory, setItemCategory] = useState<'রড' | 'সিমেন্ট' | 'রিং' | 'অন্যান্য'>('রড');
  const [rodMili, setRodMili] = useState<string>('১০ মিলি');
  const [rodBrand, setRodBrand] = useState<string>('');
  const [cementBrand, setCementBrand] = useState<string>('শাহ সিমেন্ট');
  const [ringSize, setRingSize] = useState<string>('৭" × ৭"');
  const [ringNote, setRingNote] = useState<string>('');
  ...
  // Auto-Generated Product Name Calculation
  const autoGeneratedName = useMemo(() => {
    if (itemCategory === 'রড') {
      const brand = rodBrand.trim();
      const base = `${rodMili}${brand ? ' ' + brand : ''}`.trim();
      return base.toLowerCase().endsWith('রড') ? base : `${base} রড`;
    }
    if (itemCategory === 'সিমেন্ট') {
      const brand = cementBrand.trim();
      if (!brand) return 'সিমেন্ট';
      return brand.endsWith('সিমেন্ট') ? brand : `${brand} সিমেন্ট`;
    }
    if (itemCategory === 'রিং') {
      const note = ringNote.trim() ? ` (${ringNote.trim()})` : '';
      const base = `${ringSize}${note}`.trim();
      return base.endsWith('রিং') ? base : `${base} রিং`;
    }
    return genericName.trim() || 'অন্যান্য পণ্য';
  }, [itemCategory, rodMili, rodBrand, cementBrand, ringSize, ringNote, genericName]);

  const handleCategoryChange = (cat: 'রড' | 'সিমেন্ট' | 'রিং' | 'অন্যান্য') => {
    setItemCategory(cat);
    if (cat === 'রড') {
      setItemUnit('কেজি');
      setItemAlertLimit(200);
    } else if (cat === 'সিমেন্ট') {
      setItemUnit('বস্তা');
      setItemAlertLimit(50);
    } else if (cat === 'রিং') {
      setItemUnit('পিস');
      setItemAlertLimit(100);
    } else {
      setItemUnit('পিস');
    }
  };
  ...
  const handleAddLineItem = async () => {
    const nameToUse = entryMode === 'select' && selectedProductId 
    ...
    if (entryMode === 'category') {
      const cleanName = nameToUse.trim().toLowerCase();
      const existing = products.find(p => p.name.trim().toLowerCase() === cleanName || p.name.trim().toLowerCase().includes(cleanName));
      if (existing) {
        prodId = String(existing.id);
      } else {
        prodId = '';
      }
    }
              ...
              <div className="lg:col-span-9 space-y-5">
                        ...
                        {!isNewSupplier ? (
                          ...
                          >
                            <SelectTrigger className="rounded-xl h-11 bg-slate-50 border-slate-200 font-bold text-xs">
                              <SelectValue placeholder="কোম্পানি / সরবরাহকারী খুঁজুন..." />
                            </SelectTrigger>
                            <SelectContent className="font-bengali max-h-60 text-xs font-bold">
                              {suppliers.map(s => (
                                <SelectItem key={s.id} value={s.id}>
                                  {s.businessName || s.name} {s.phone ? `(${s.phone})` : ''}
                                </SelectItem>
                              ))}
                            </SelectContent>
                          </Select>
                ...
                <Card className="bg-white border-slate-200/80 rounded-2xl shadow-xs">
                  <CardContent className="p-5 space-y-4">
                    ...
                    {/* Single Unified Product Entry Form */}
                    <div className="p-4 bg-slate-50/90 border border-slate-200 rounded-2xl space-y-4 font-bengali">
                      <div className="flex items-center justify-between border-b border-slate-200/70 pb-3">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <h4 className="text-xs font-black text-slate-800">নতুন পণ্য যুক্ত করুন</h4>
                            <p className="text-[10px] text-slate-500 font-semibold">বিভাগ বেছে তথ্য দিলে স্বয়ংক্রিয় নাম তৈরি হয়ে স্টক থাকলে আপগ্রেড হবে এবং নতুন হলে তৈরি হবে</p>
                          </div>
                        </div>
                      </div>

                      {/* Category Selector & Dynamic Fields */}
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div className="space-y-1">
                          <Label className="text-[11px] font-bold text-slate-600">বিভাগ (Category)</Label>
                          <Select value={itemCategory} onValueChange={(val: any) => handleCategoryChange(val)}>
                            ...
                            <SelectContent className="font-bengali text-xs font-bold">
                              <SelectItem value="রড">রড 🏗️</SelectItem>
                              <SelectItem value="সিমেন্ট">সিমেন্ট 🧱</SelectItem>
                              <SelectItem value="রিং">রিং ⭕</SelectItem>
                              <SelectItem value="অন্যান্য">অন্যান্য 📦</SelectItem>
                            </SelectContent>
                          </Select>
                        </div>

                        {/* Dynamic Fields for Rod (Image 1) */}
                        {itemCategory === 'রড' && (
                          <>
                            <div className="space-y-1">
                              <Label className="text-[11px] font-bold text-slate-600">রড মিলি (সাইজ)</Label>
                              <Select value={rodMili} onValueChange={(v: string | null) => setRodMili(v || '')}>
                                <SelectTrigger className="rounded-xl h-10 bg-white border-slate-200 text-xs font-bold">
                                  <SelectValue />
                                </SelectTrigger>
                                <SelectContent className="font-bengali text-xs font-bold">
                                  <SelectItem value="৮ মিলি">৮ মিলি</SelectItem>
                                  <SelectItem value="১০ মিলি">১০ মিলি</SelectItem>
                                  <SelectItem value="১২ মিলি">১২ মিলি</SelectItem>
                                  <SelectItem value="১৬ মিলি">১৬ মিলি</SelectItem>
                                  <SelectItem value="২০ মিলি">২০ মিলি</SelectItem>
                                  <SelectItem value="২৫ মিলি">২৫ মিলি</SelectItem>
                                  <SelectItem value="৩২ মিলি">৩২ মিলি</SelectItem>
                                </SelectContent>
                              </Select>
                            </div>

                            <div className="space-y-1">
                              <Label className="text-[11px] font-bold text-slate-600">রড ব্র্যান্ড</Label>
                              <Select value={rodBrand} onValueChange={(v: string | null) => setRodBrand(v || '')}>
                                <SelectTrigger className="rounded-xl h-10 bg-white border-slate-200 text-xs font-bold">
                                  <SelectValue placeholder="ব্র্যান্ড বাছুন..." />
                                </SelectTrigger>
                                <SelectContent className="font-bengali text-xs font-bold">
                                  <SelectItem value="">ব্র্যান্ড বাছুন (ঐচ্ছিক)</SelectItem>
                                  <SelectItem value="BSRM">BSRM (বিএসআরএম)</SelectItem>
                                  <SelectItem value="KSRM">KSRM (কেএসআরএম)</SelectItem>
                                  <SelectItem value="AKS">AKS (আবুল খায়ের)</SelectItem>
                                  <SelectItem value="GPH">GPH ইস্পাত</SelectItem>
                                  <SelectItem value="Anwar">আনোয়ার ইস্পাত</SelectItem>
                                  <SelectItem value="Shahriar">শহরিয়ার স্টিল</SelectItem>
                                  <SelectItem value="Local">লোকাল রড</SelectItem>
                                </SelectContent>
                              </Select>
                            </div>
                          </>
                        ...
                        {/* Dynamic Fields for Cement (Image 2) */}
                        {itemCategory === 'সিমেন্ট' && (
                          <div className="sm:col-span-2 space-y-1">
                            <Label className="text-[11px] font-bold text-slate-600">সিমেন্ট ব্র্যান্ড নির্বাচন করুন</Label>
                            <Select value={cementBrand} onValueChange={(v: string | null) => setCementBrand(v || '')}>
                              <SelectTrigger className="rounded-xl h-10 bg-white border-slate-200 text-xs font-bold">
                                <SelectValue placeholder="সিমেন্ট ব্র্যান্ড বাছুন..." />
                              </SelectTrigger>
                              <SelectContent className="font-bengali text-xs font-bold">
                                <SelectItem value="শাহ সিমেন্ট">শাহ সিমেন্ট</SelectItem>
                                <SelectItem value="বসুন্ধরা সিমেন্ট">বসুন্ধরা সিমেন্ট</SelectItem>
                                <SelectItem value="ক্রাউন সিমেন্ট">ক্রাউন সিমেন্ট</SelectItem>
                                <SelectItem value="ফ্রেশ সিমেন্ট">ফ্রেশ সিমেন্ট</SelectItem>
                                <SelectItem value="আকিজ সিমেন্ট">আকিজ সিমেন্ট</SelectItem>
                                <SelectItem value="সেভেন রিং সিমেন্ট">সেভেন রিং সিমেন্ট</SelectItem>
                                <SelectItem value="অন্যান্য">অন্যান্য সিমেন্ট</SelectItem>
                              </SelectContent>
                            </Select>
                          </div>
                        )}

                        {/* Dynamic Fields for Ring (Image 3) */}
                        {itemCategory === 'রিং' && (
                          <>
                            <div className="space-y-1">
                              <Label className="text-[11px] font-bold text-slate-600">রিং এর সাইজ (Ring Size)</Label>
                              <Select value={ringSize} onValueChange={(v: string | null) => setRingSize(v || '')}>
                                <SelectTrigger className="rounded-xl h-10 bg-white border-slate-200 text-xs font-bold">
                                  <SelectValue />
                                </SelectTrigger>
                                <SelectContent className="font-bengali text-xs font-bold">
                                  <SelectItem value='৭″ × ৭″'>৭″ × ৭″</SelectItem>
                                  <SelectItem value='৭″ × ৯″'>৭″ × ৯″</SelectItem>
                                  <SelectItem value='৮″ × ৮″'>৮″ × ৮″</SelectItem>
                                  <SelectItem value='৮″ × ১০″'>৮″ × ১০″</SelectItem>
                                  <SelectItem value='১০″ × ১০″'>১০″ × ১০″</SelectItem>
                                </SelectContent>
                              </Select>
                            </div>

                            <div className="space-y-1">
                              <Label className="text-[11px] font-bold text-slate-600">ব্র্যান্ড / নোট</Label>
                              <Input
                                value={ringNote}
                                onChange={e => setRingNote(e.target.value)}
                                placeholder="যেমন: BSRM / লোকাল"
                                className="rounded-xl h-10 bg-white text-xs font-bold"
                              />
                            </div>
                          </>
                        )}

                        {itemCategory === 'অন্যান্য' && (
                          <div className="sm:col-span-2 space-y-1">
                            <Label className="text-[11px] font-bold text-slate-600">পণ্যের নাম</Label>
                            <Input
                              value={genericName}
                      ...
                      {/* Auto-Generated Product Name Banner */}
                      <div className="p-3 bg-white rounded-xl border border-slate-200/90 flex items-center justify-between shadow-2xs">
                        <span className="text-xs font-bold text-slate-600">স্বয়ংক্রিয় তৈরি পণ্যের নাম:</span>
                        <span className="text-xs font-black text-orange-600 bg-orange-50 border border-orange-200 px-3 py-1 rounded-lg">
                          {autoGeneratedName}
                        </span>
                      </div>

                      {/* Prices, Qty, Unit, Alert Limit Inputs */}
                      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                        <div className="space-y-1">
                          <Label className="text-[11px] font-bold text-slate-600">ক্রয় মূল্য (৳)</Label>
                          <Input 
                            type="number" 
                            min="0" 
                            ...
                            placeholder="0"
                            className="rounded-xl h-10 bg-white font-bold text-xs text-orange-600"
                          />
                        </div>
                        ...
                        <div className="space-y-1">
                          ...
                          <Input 
                            type="number" 
                            min="0" 
                            value={isNaN(itemSellPrice) ? '' : itemSellPrice} 
                            onChange={e => setItemSellPrice(parseFloat(e.target.value) || 0)} 
                            placeholder="0"
                            className="rounded-xl h-10 bg-white font-bold text-xs text-emerald-600"
                          />
                        </div>
                        ...
                        <div className="space-y-1">
                          ...
                          <Input 
                            ...
                            placeholder="0"
                            className="rounded-xl h-10 bg-white font-bold text-xs"
                          />
                        </div>

                        <div className="space-y-1">
                          <Label className="text-[11px] font-bold text-slate-600">একক (Unit)</Label>
                          <Select value={itemUnit} onValueChange={(v: string | null) => setItemUnit(v || 'কেজি')}>
                            <SelectTrigger className="rounded-xl h-10 bg-white border-slate-200 text-xs font-bold">
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent className="font-bengali text-xs font-bold">
                              ...
                              <SelectItem value="ফিট">ফিট</SelectItem>
                            </SelectContent>
                          </Select>
                        </div>

                        <div className="space-y-1 col-span-2 sm:col-span-1">
                          <Label className="text-[11px] font-bold text-slate-600">সতর্কতা সীমা (Alert Limit)</Label>
                          <Input 
                            type="number" 
                            min="0" 
                            value={isNaN(itemAlertLimit) ? '' : itemAlertLimit} 
                            onChange={e => setItemAlertLimit(parseFloat(e.target.value) || 0)} 
                            placeholder="50"
                            className="rounded-xl h-10 bg-white font-bold text-xs"
                          />
                        </div>
                      ...
                      <Button 
                        type="button" 
                        onClick={handleAddCategoryProductToCart} 
                        className="w-full bg-orange-600 hover:bg-orange-700 text-white h-11 rounded-xl font-black text-sm shadow-md shadow-orange-600/20 active:scale-98 transition-all cursor-pointer"
                      >
                        + কার্টে পণ্য যোগ করুন (Add Product to Invoice)
                      </Button>
                    </div>
